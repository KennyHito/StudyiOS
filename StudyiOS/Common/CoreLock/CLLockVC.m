...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.bottomSpace.constant = KSafeAreaHeight;
    //控制器准备
    [self vcPrepare];
    //数据传输
    [self dataTransfer];
...
-(void)event{
    ...
    self.lockView.setPWTwiceSameBlock = ^(NSString *pwd){
        ...
        if(CoreLockTypeModifyPwd == self.type){
    ...
    self.lockView.verifyPwdBlock = ^(NSString *pwd){
        ...
        if(res){//密码一致
            ...
            if(CoreLockTypeVeryfiPwd == self.type){
                //禁用交互
                self.view.userInteractionEnabled = NO;
            }else if (CoreLockTypeModifyPwd == self.type){//修改密码
                [self.label showNormalMsg:CoreLockPWDTitleFirst];
